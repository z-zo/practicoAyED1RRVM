...
#--------------------------------------------
"""
En el desarrollo del Proyecto tendrán que implementar: 
• GIT/GITHUB
• Datos estructurados (Listas y matrices). 
• Funciones lambda. Por comprensión y slicing.
• Programación modular - funciones con módulo propio
...
def mostrarLista(legajos, edades, nombres, liLev):
    ...
    print("---------------------------------------------------------------------------------")
    print("\nLegajo | Edad | nombre | Intento 1 | Intento 2 | Intento 3 | Promedio ")
    print("---------------------------------------------------------------------------------")
    for i in range(len(legajos)):
        promedio = round((liLev[i][0] + liLev[i][1] + liLev[i][2]) / 3)
        print("%6d | %4d | %4s | %3d kg    | %3d kg    | %3d kg    | %3d kg" % (legajos[i], edades[i], nombres[i].title(), liLev[i][0], liLev[i][1], liLev[i][2], promedio))
        print("---------------------------------------------------------------------------------")
...
def main():
    ...
    if len(legajos) != 0:
        ...
        nombreBuscar = input("Juez de levantamiento, por favor, Ingrese el nombre del atleta para consultar el intento con su levantamiento mínimo: ").lower()
        ...
        while minimoLevantamientos == 201 :
            print("No se encontraron levantamientos para el nombre", nombreBuscar)
            nombreBuscar =input("Por favor, Re-Ingrese nuevamente el nombre para consultar el intento con su levantamiento mínimo: ")
            minimoLevantamientos = encontrarMinimo(liLev, nombres, nombreBuscar)

        # Cuando se encuentre el mínimo de levantamientos, imprimir el resultado.
        if(minimoLevantamientos != -1):
            print("El intento con su levantamiento mínimo para el atleta con nombre", nombreBuscar, "es:", minimoLevantamientos, "Kg")
            print("--------------------------------------------------------------------------------")
